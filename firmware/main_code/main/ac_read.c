...
// Struct to hold PZEM-004T readings
typedef struct {
    float voltage;
    float current;
    float power;
...
int pzem_read_response(uint8_t *buf, size_t len) {
...
// Function to parse PZEM-004T measurement data and populate the struct
PzemReadings pzem_parse_data(uint8_t *data) {
    PzemReadings readings;

    readings.voltage = ((data[3] << 8) | data[4]) / 10.0;
    readings.current = ((data[5] << 8) | data[6]) / 1000.0;
    readings.power = ((data[7] << 8) | data[8]) / 10.0;
    readings.energy = ((data[9] << 16) | (data[10] << 8) | data[11]) / 1000.0;
    readings.frequency = ((data[12] << 8) | data[13]) / 10.0;
    readings.power_factor = ((data[14] << 8) | data[15]) / 100.0;

    ESP_LOGI(TAG, "Voltage: %.1f V", readings.voltage);
    ESP_LOGI(TAG, "Current: %.3f A", readings.current);
    ESP_LOGI(TAG, "Power: %.1f W", readings.power);
    ESP_LOGI(TAG, "Energy: %.3f kWh", readings.energy);
    ESP_LOGI(TAG, "Frequency: %.1f Hz", readings.frequency);
    ESP_LOGI(TAG, "Power Factor: %.2f", readings.power_factor);

    return readings;
}

// Function to read data from the PZEM-004T and return the readings
PzemReadings pzem_read_data() {
    PzemReadings readings = {0}; // Initialize all fields to 0
    ...
    int len = pzem_read_response(response, sizeof(response));

    if (len > 0) {
        // Parse and store the measurement data
        readings = pzem_parse_data(response);
    } else {
